...
class AdminMonetaryFlowScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AdminMonetaryFlowScreen> createState() =>
      _AdminMonetaryFlowScreenState();
}

class _AdminMonetaryFlowScreenState
    extends ConsumerState<AdminMonetaryFlowScreen> {
  @override
  void initState() {
    super.initState();
  ...
  Widget _buildContent(AdminMonetaryFlowState state) {
    ...
    final totalPlayerBalance =
        (data['total_player_balance'] as num?)?.toDouble() ?? 0.0;
    final totalTokenBalance =
        (data['total_token_balance'] as num?)?.toDouble() ?? 0.0;
    ...
    for (final entry in flowTimeseries) {
      if (entry is Map<String, dynamic>) {
        ...
        timeLabels.add(
          AnalyticsFormat.shortDate(entry['timestamp']?.toString() ?? ''),
        );
      }
    }
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return GridView.count(
          ...
          children: [
            _tip(
              'Montant total déposé par les joueurs.',
              AdminMetricCard(
                title: 'Dépôts',
                value: AnalyticsFormat.amountPrecise(
                  deposits['total'] is int ? deposits['total'] / 100 : 0,
                ),
            ...
            _tip(
              'Montant total retiré par les joueurs.',
              AdminMetricCard(
                title: 'Retraits',
                value: AnalyticsFormat.amountPrecise(
                  withdrawals['total'] is int ? withdrawals['total'] / 100 : 0,
                ),
            ...
            _tip(
              'Mises totales engagées dans les jeux.',
              AdminMetricCard(
                title: 'Mises',
                value: AnalyticsFormat.amountPrecise(bets['total']),
                icon: Icons.casino,
            ...
            _tip(
              'Gains totaux reversés aux joueurs.',
              AdminMetricCard(
                title: 'Gains',
                value: AnalyticsFormat.amountPrecise(winnings['total']),
            ...
            _tip(
              ...
              AdminMetricCard(
                title: 'Commissions',
                value: AnalyticsFormat.amountPrecise(commissions['total']),
                icon: Icons.percent,
                color: NeonColors.primary,
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return GridView.count(
          ...
          children: [
            _tip(
              'Revenu brut des jeux (GGR) : Mises − Gains.',
              AdminMetricCard(
                title: 'GGR',
                value: AnalyticsFormat.amountPrecise(ggr),
                icon: Icons.attach_money,
                color: NeonColors.success,
              ),
            ),
            _tip(
              'Revenu net des jeux (NGR) : GGR − Commissions.',
              AdminMetricCard(
                title: 'NGR',
                value: AnalyticsFormat.amountPrecise(ngr),
                icon: Icons.money_off,
                color: ngr >= 0 ? NeonColors.success : NeonColors.error,
              ),
            ),
            _tip(
              'Marge maison : GGR / Mises × 100.',
              AdminMetricCard(
                title: 'Marge',
                value: '${houseEdge.toStringAsFixed(1)}%',
                icon: Icons.trending_up,
                color: NeonColors.primary,
              ),
            ),
            _tip(
              ...
              AdminMetricCard(
                ...
                value: '${payoutRatio.toStringAsFixed(1)}%',
                icon: Icons.replay,
                color: NeonColors.accent,
              ),
            ),
            _tip(
              'Position nette : Dépôts − Retraits.',
              AdminMetricCard(
                title: 'Flux net',
                value: AnalyticsFormat.amountPrecise(netFlow / 100),
                icon: netFlow >= 0 ? Icons.north_east : Icons.south_east,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              const AnalyticsLegendDot(NeonColors.success, 'Entrées'),
              const SizedBox(width: 16),
              const AnalyticsLegendDot(NeonColors.error, 'Sorties'),
              const Spacer(),
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: AdminLineChart(
                    data: outflows,
                    lineColor: NeonColors.error,
                    height: 160,
                    showDots: outflows.length <= 30,
                  ),
                ),
              ],
            ),
  ...
  Widget _buildComparisonChart(
    ...
    final maxVal = [deposits, withdrawals, bets, winnings, commissions]
        .reduce((a, b) => a > b ? a : b);
    if (maxVal <= 0) {
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            width: 80,
            child: Text(
              AnalyticsFormat.amountPrecise(value),
              style: const TextStyle(
  ...
  Widget _buildSankeyDiagram(
    ...
    final maxFlow = [deposits, bets, winnings, withdrawals]
        .where((v) => v > 0)
        ...
        child: Row(
          children: [
            ...
            SizedBox(
              width: 80,
              child: Text(
                AnalyticsFormat.amountPrecise(amount),
                style: const TextStyle(
                  color: NeonColors.textPrimary,
                  fontSize: 10,
                ),
  ...
  Widget _buildPlatformBalance(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          _tip(
            'Solde total (wiga) de tous les joueurs.\nValeur exacte : ${(fcfaBalance / 100).toStringAsFixed(0)} wiga',
            _balanceItem(
              'Solde Wiga',
              fcfaBalance / 100,
              NeonColors.success,
          ...
          _tip(
            'Solde en wiga (tokens) de tous les joueurs.\nValeur exacte : ${tokenBalance.toStringAsFixed(0)} wiga',
            _balanceItem('Solde Wiga', tokenBalance, NeonColors.accent),
          ),
          Container(width: 1, height: 40, color: NeonColors.border),
          _tip(
            'Position nette : Dépôts − Retraits.\nValeur exacte : ${(netFlow / 100).toStringAsFixed(0)} wiga',
            _balanceItem(
              'Flux net',
              netFlow / 100,
              netFlow >= 0 ? NeonColors.success : NeonColors.error,
            ),
          ),
        ],
      ),
  ...
  Widget _buildVelocityCards(
    double velocity,
    double avgBalance,
    Map<String, dynamic> deposits,
    Map<String, dynamic> withdrawals,
  ) {
    final totalTxCount = ((deposits['count'] as num?)?.toInt() ?? 0) +
        ((withdrawals['count'] as num?)?.toInt() ?? 0);
    return Row(
      children: [
        Expanded(
          child: _tip(
            'Ratio volume/solde. Plus c\'est élevé, plus l\'argent circule.',
            _velocityCard(
              'Vélocité',
              '${velocity.toStringAsFixed(1)}x',
              NeonColors.primary,
            ),
          ),
        ),
        ...
        Expanded(
          child: _tip(
            'Solde moyen détenu par les joueurs.',
            _velocityCard(
              'Solde moyen',
              AnalyticsFormat.amountPrecise(avgBalance),
              NeonColors.accent,
            ),
          ),
        ),
        ...
        Expanded(
          child: _tip(
            'Nombre total de transactions (dépôts + retraits).',
            _velocityCard(
              'Transactions',
              '$totalTxCount',
              NeonColors.secondary,
          ...
          SingleChildScrollView(
            ...
            child: DataTable(
              ...
              rows: movements.take(10).map((m) {
                ...
                final isPositive =
                    type == 'deposit' || type == 'winnings' || type == 'refund';
                return DataRow(
                  cells: [
                    DataCell(
                      Text(
                        m['username'] as String? ?? '#${m['user_id']}',
                        style: const TextStyle(
                          color: NeonColors.textPrimary,
                          fontSize: 11,
                        ),
                    ...
                    DataCell(
                      Text(
                        AnalyticsFormat.amountPrecise(m['amount']),
                        style: TextStyle(
                          color: isPositive
                              ? NeonColors.success
                              : NeonColors.error,
                    ...
                    DataCell(
                      Text(
                        AnalyticsFormat.date(m['inserted_at'] ?? m['date']),
                        style: const TextStyle(
                          color: NeonColors.textMuted,
                          fontSize: 10,
                        ),
